		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					IF (SC6->C6_FSIMPOB == "1") //SE TEM MENSAGEM  
						...
						IF (!EMPTY((cAliasSD2)->D2_LOTECTL))
							...
							c_DesComp:= UPPER(SZ1->Z1_FSOBS) + " LOTE: "+ALLTRIM((cAliasSD2)->D2_LOTECTL)+" VALIDADE: "+Substr((cAliasSD2)->D2_DTVALID,7,2) +"/"+Substr((cAliasSD2)->D2_DTVALID,5,2)+"/"+ Substr((cAliasSD2)->D2_DTVALID,1,4)
												
						ELSE
						
							c_DesComp:= UPPER(SZ1->Z1_FSOBS)	
					...
					IF(Empty(SC6->C6_DESCRI))
						...
						IF n_MenLot = 3 //NAO TEM MENSAGEM E TEM LOTE
							...
							c_Desc+= " LOTE: "+(cAliasSD2)->D2_LOTECTL+" VALIDADE: "+Substr((cAliasSD2)->D2_DTVALID,7,2) +"/"+Substr((cAliasSD2)->D2_DTVALID,5,2)+"/"+ Substr((cAliasSD2)->D2_DTVALID,1,4)
						
						ENDIF
							
					ELSE
					
						c_Desc:= ALLTRIM(SC6->C6_DESCRI) 
