...
WSMETHOD GetXmlNFSe WSRECEIVE CodEmp,CodFil,CNPJ,NumeroDaNFS WSSEND XMLNFe WSSERVICE u_wsGetXmlNFSe
	...
	CATCHEXCEPTION USING oException lReturn := .F.
	...
	ENDEXCEPTION CATCHEXCEPTION USING oException lReturn := .F.
	...
	ENDEXCEPTION RESET ENVIRONMENT
...
Static Function GetIdEnt( cError )
	...
	BEGIN SEQUENCE
		...
		DEFAULT lMethodOk := .F.
		IF !( lMethodOk ) cError := IF( Empty( GetWscError(3) ) , GetWscError(1) , GetWscError(3) )
			BREAK
		EndIF
