...
If lDataFree
	...
	cQry+=" INNER JOIN " + RetSqlName("CQD")+" CQD ON"
	...
	cQry+=" INNER JOIN " + RetSqlName("CTE")+" CTE ON"
	cQry+=" CTG_FILIAL = CTE.CTE_FILIAL"
	cQry+=" AND CTG_CALEND = CTE.CTE_CALEND"
	cQry+=" WHERE CTG_DTINI <= '"+DTOS(dData)+"'"
	cQry+=" AND CTG_DTFIM >= '"+DTOS(dData)+"'"
	cQry+=" AND CQD.CQD_PROC IN "+FormatIn(cProc,"/")
	cQry+=" AND CTG_FILIAL = '"+xFilial("CTG")+"'"
