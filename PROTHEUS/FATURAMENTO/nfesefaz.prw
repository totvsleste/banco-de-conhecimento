...
Local c_DesComp		:= ""  //FABRICA DE SOFTWARE - 13/02/2012 - VARIACAO QUE GUARDA A MENSAGEM DO CAMPO MENO NO PEDIDOD E VENDA
Local c_Desc		:= ""  //FABRICA DE SOFTWARE - 13/02/2012 - VARIAVEL QUE GUARDA A DESC DO PRPODUTO
Local n_MenLot		:= 0   //FABRICA DE SOFTWARE - CONTROLA LOTE E MENSAGEM 																	//respectivamente as tags "dest", "entrega" e "retirada" no XML
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
			...
			Else
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSD2
							...
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_DTVALID %Exp:cField%
							FROM %Table:SD2% SD2
							WHERE
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						Else
							cMensCli += AllTrim(SC5->C5_MENNOTA)//cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_MENNOTA))
						EndIf
						//Fim Medtest
					...
					//³TRATAMENTO PARA IMPRIMIR A DESCRICAO COMPLEMENTAR DO PRODUTO NO DANFE BUSCANDO DO PEDIDO DE VENDA.³
					//³FABRICA DE SOFTWARE - 13/02/2012                                                                  ³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

				    //Inicio MEDTEST
					IF (SC6->C6_FSIMPOB == "1") //SE TEM MENSAGEM
						DBSELECTAREA("SZ1")
						DBSETORDER(3)
						DBSEEK(XFILIAL("SZ1")+SC6->C6_FSCDMEN,.T.)
						//c_DesComp:= "LOTE: "+ALLTRIM((cAliasSD2)->D2_LOTECTL)+" VALIDADE: "+Substr((cAliasSD2)->D2_DTVALID,7,2) +"/"+Substr((cAliasSD2)->D2_DTVALID,5,2)+"/"+ Substr((cAliasSD2)->D2_DTVALID,1,4)+" - "+UPPER(SZ1->Z1_FSOBS)

						IF (!EMPTY((cAliasSD2)->D2_LOTECTL))

							cDescProd:= UPPER(SZ1->Z1_FSOBS) + " LOTE: "+ALLTRIM((cAliasSD2)->D2_LOTECTL)+" VALIDADE: "+Substr((cAliasSD2)->D2_DTVALID,7,2) +"/"+Substr((cAliasSD2)->D2_DTVALID,5,2)+"/"+ Substr((cAliasSD2)->D2_DTVALID,1,4)

						ELSE

							cDescProd:= UPPER(SZ1->Z1_FSOBS)

						ENDIF

					ELSE //QUANDO NAO TEM MENSAGEM TESTA SE TEM LOTE

						IF (!EMPTY((cAliasSD2)->D2_LOTECTL)) //SE TEM LOTE
					...
					IF(Empty(SC6->C6_DESCRI))
						...
						IF n_MenLot = 3 //NAO TEM MENSAGEM E TEM LOTE

							cDescProd+= " LOTE: "+(cAliasSD2)->D2_LOTECTL+" VALIDADE: "+Substr((cAliasSD2)->D2_DTVALID,7,2) +"/"+Substr((cAliasSD2)->D2_DTVALID,5,2)+"/"+ Substr((cAliasSD2)->D2_DTVALID,1,4)
					...
					ENDIF
		            //Fim MEDTEST
		            //ESTE TRECHO ESTAVA NA PARTE ONDE ESTA A VARIAVEL c_Desc
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								...
								aTail(aICMUFDest) := {IIf(CD2->CD2_BC > 0,CD2->CD2_BC, 0),; //[1]vBCUFDest
									...
									IIf(CD2->(FieldPos("CD2_VFCP")) > 0 .and. CD2->CD2_VFCP > 0,CD2->CD2_VFCP,CD2->CD2_VLTRIB),; //IIf(CD2->(FieldPos("CD2_VFCP")) > 0 .and. CD2->CD2_VFCP > 0,CD2->CD2_VFCP,0),;//[6]vFCPUFDest
